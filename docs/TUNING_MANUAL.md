...
| Hermes / OpenClaw / Claude Desktop mount | `dispatcher/identity_mcp.py`, `docs/MOUNTING.md` | **WORKING MCP SERVER (2026-07-19)** | Generic stdio MCP wrapper, 4-tool closed surface, boot conformance gate (refuses to mount if closed track/signer/audit not intact). Proven by `tools/mcp_roundtrip.py` (live protocol round trip: describe, gated submit, unsigned money rejected, signed executed, chain verified). Full host install not yet exercised against a live Hermes/OpenClaw instance - the MCP protocol is, which is what they speak. |
